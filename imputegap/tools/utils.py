...
def load_parameters(
    ...
    if query == "default":
        if path is None:
            filepath = importlib.resources.files("imputegap.env").joinpath(
                "./default_values.toml"
            )
    ...
    if algorithm == "cdrec":
        ...
        return (lr, batch_size, patience, epochs, workers, tr_ratio)
...
def config_impute_algorithm(incomp_data, algorithm, verbose=True, **algo_params):
    ...
    #    imputer = Imputation.MyFamily.NewAlg(incomp_data)
        ...
        raise ValueError(
            f"(IMP) Algorithm '{algorithm}' not recognized, please choose your algorithm from this list:\n\t{TimeSeries().algorithms}"
        )
        imputer = None
...
def save_optimization(
    optimal_params, algorithm="cdrec", dataset="", optimizer="b", file_name=None
):
    """
    ...
    if file_name is None:
        file_name = (
            "./imputegap_assets/params/optimal_parameters_"
            + str(optimizer)
            + "_"
            + str(dataset)
            + "_"
    ...
    if algorithm == "cdrec" or algorithm == "CDRec":
        ...
        params_to_save = {"learning_neighbors": int(optimal_params[0])}

    elif algorithm == "iterative_svd" or algorithm == "IterativeSVD":
        params_to_save = {"rank": int(optimal_params[0])}
    elif algorithm == "grouse" or algorithm == "GROUSE":
        params_to_save = {"max_rank": int(optimal_params[0])}
        ...
        params_to_save = {"n_estimators": int(optimal_params[0]), "seed": 42}
    elif algorithm == "miss_net":
        params_to_save = {
...
def config_contamination(
    ...
    if ptn == "mcar" or ptn == "missing_completely_at_random":
        incomp_data = ts.Contamination.mcar(
            input_data=ts.data,
            rate_dataset=dataset_rate,
            rate_series=series_rate,
        ...
        incomp_data = ts.Contamination.aligned(
            input_data=ts.data,
            rate_dataset=dataset_rate,
            rate_series=series_rate,
            offset=offset,
        ...
        ptn == "ps"
        or ptn == "percentageshift"
        or ptn == "scattered"
        or ptn == "scatter"
    ):
        incomp_data = ts.Contamination.scattered(
            input_data=ts.data,
            rate_dataset=dataset_rate,
            rate_series=series_rate,
            offset=offset,
        ...
        incomp_data = ts.Contamination.disjoint(
            input_data=ts.data,
            rate_series=dataset_rate,
            limit=1,
            offset=offset,
            verbose=verbose,
        )
    elif ptn == "overlap":
        incomp_data = ts.Contamination.overlap(
            input_data=ts.data,
            rate_series=dataset_rate,
            limit=limit,
            shift=shift,
        ...
        incomp_data = ts.Contamination.gaussian(
            input_data=ts.data,
            rate_dataset=dataset_rate,
            rate_series=series_rate,
            std_dev=std_dev,
        ...
        incomp_data = ts.Contamination.distribution(
            input_data=ts.data,
            rate_dataset=dataset_rate,
            rate_series=series_rate,
        ...
        incomp_data = ts.Contamination.blackout(
            input_data=ts.data, series_rate=dataset_rate, offset=offset, verbose=verbose
        )
    else:
        raise ValueError(
...
def __marshal_as_numpy_column(__ctype_container, __py_sizen, __py_sizem):
    ...
    """
    __numpy_marshal = (
        __numpy_import.array(__ctype_container).reshape(__py_sizem, __py_sizen).T
    )
...
def display_title(
    ...
    aut="Quentin Nater",
    lib="ImputeGAP",
    university="University Fribourg",
):
    """
...
def verification_limitation(percentage, low_limit=0.01, high_limit=1.0):
    ...
    if low_limit <= percentage <= high_limit:
        ...
        print(
            f"The percentage {percentage} is between 1 and 100. Dividing by 100 to convert to a decimal."
        )
...
def dl_integration_transformation(
    input_matrix,
    tr_ratio=0.8,
    ...
    cont_data_matrix, new_mask, error = prepare_testing_set(
    ...
    mask_test, mask_valid, nbr_nans = split_mask_bwt_test_valid(
        cont_data_matrix,
        test_rate=split_ts,
        valid_rate=split_val,
        nan_val=nan_val,
        verbose=verbose,
        seed=seed,
    )
    mask_train = generate_random_mask(
        gt=cont_data_matrix,
        mask_test=mask_test,
        mask_valid=mask_valid,
        droprate=inside_tr_cont_ratio,
        offset=offset,
        verbose=verbose,
        seed=seed,
    )
...
def prepare_fixed_testing_set(
    incomp_m, tr_ratio=0.8, offset=0.05, block_selection=True, verbose=True
):
    """
    ...
    for row in range(data_matrix_cont.shape[0]):
        available_mask[row, :offset_vals] = (
            False  # protect leftmost `offset` columns in each row
        )
    ...
    if n_new_nans > 0:
        if block_selection:
            ...
            chosen_indices = available_indices[
                np.random.choice(len(available_indices), n_new_nans, replace=False)
            ]
    ...
    assert abs(missing_ratio - target_ratio) < 0.01, (
        f"Missing ratio {missing_ratio} is not {target_ratio}"
    )
    assert abs(observed_ratio - tr_ratio) < 0.01, (
        f"Missing ratio {observed_ratio} is not {tr_ratio}"
    )
    ...
    if verbose:
        print(
            f"(DL): TEST-SET > Test set fixed to {int(round(target_ratio * 100))}% of the dataset, for {target_n_nan} values, add test values: {n_new_nans}"
        )

    return new_m, new_mask


def split_mask_bwt_test_valid(
    data_matrix, test_rate=0.8, valid_rate=0.2, nan_val=None, verbose=False, seed=42
):
    """
    ...
    assert np.isclose(test_rate + valid_rate, 1.0), (
        "test_rate and valid_rate must sum to 1.0"
    )
...
def generate_random_mask(
    gt, mask_test, mask_valid, droprate=0.2, offset=None, verbose=False, seed=42
):
    """
    ...
    assert gt.shape == mask_test.shape == mask_valid.shape, (
        "All input matrices must have the same shape"
    )
    ...
    assert overlap == 0, (
        f"Overlap detected between training and test masks: {overlap} entries."
    )
...
def prepare_testing_set(
    incomp_m, original_missing_ratio, block_selection=True, tr_ratio=0.8, verbose=True
):
    import numpy as np
    ...
    if abs((1 - tr_ratio) - original_missing_ratio) > 0.01:
        new_m, new_mask = prepare_fixed_testing_set(
            incomp_m, tr_ratio, block_selection=block_selection, verbose=verbose
        )

        if verbose:
            print(
                f"(DL) TEST-SET : building of the test set to reach a fix ratio of {1 - tr_ratio:.2%}..."
            )
            final_ratio = get_missing_ratio(new_m)
            print(
                f"(DL) TEST-SET : final artificially missing ratio for test set = {final_ratio:.2%}"
            )
            print(
                f"(DL) TEST-SET : final number of rows with NaN values = {np.sum(np.isnan(new_m).any(axis=1))}"
            )
            print(
                f"(DL) TEST-SET : final artificially missing numbers = {np.sum(new_mask)}\n"
            )
...
def compute_rank_check(M, rank, verbose=True):
    ...
    if rank >= M - 1:
        if verbose:
            print(
                f"ERROR: Rank choosen to high for the number of series: {rank} >= {M}.\n\tRank reduced to 2."
            )
        return 2
    else:
        return rank
...
def load_share_lib(name="lib_cdrec", lib=True, verbose=True):
    ...
    if lib:
        lib_path = importlib.resources.files("imputegap.algorithms.lib").joinpath(
            "./" + str(name) + ext
        )
    else:
...
def list_of_optimizers():
    return sorted(
        ["ray_tune", "bayesian", "particle_swarm", "successive_halving", "greedy"]
    )
...
def list_of_downstreams_darts():
    return sorted(["nbeats", "xgboost", "lightgbm", "lstm", "deepar", "transformer"])


def list_of_extractors():
    return sorted(["pycatch", "tsfel", "tsfresh"])
