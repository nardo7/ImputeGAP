...
from imputegap.wrapper.AlgoPython.GRIN.lib import config, datasets, fillers
from imputegap.wrapper.AlgoPython.GRIN.lib.data.datamodule import (
    SpatioTemporalDataModule,
)
from imputegap.wrapper.AlgoPython.GRIN.lib.data.imputation_dataset import (
...
from imputegap.wrapper.AlgoPython.GRIN.lib.nn.utils.metrics import (
    MaskedMAE,
    MaskedMAPE,
    MaskedMRE,
    MaskedMSE,
...
def recoveryGRIN(
    ...
    cont_data_matrix, mask_train, mask_test, mask_val, error = (
        utils.dl_integration_transformation(
            input,
            tr_ratio=tr_ratio,
            inside_tr_cont_ratio=0.4,
            split_ts=1,
            split_val=0,
            nan_val=0.0,
            prevent_leak=True,
    ...
    if verbose:
        ...
        print(
            f"🔍 Training Indices: {len(train_idxs) if train_idxs is not None else 0}"
        )
    ...
    model_kwargs = parser_utils.filter_args(
        args={**split_conf, **additional_model_hparams}, target_cls=model_cls
    )

    # loss and metrics
    loss_fn = MaskedMetric(
        metric_fn=getattr(F, loss_fn), metric_kwargs={"reduction": "none"}
    )
    ...
    filler_kwargs = parser_utils.filter_args(
        args={**split_conf, **additional_filler_hparams},
    ...
    trainer = pl.Trainer(
        ...
        else "cpu",  # Automatically detect GPU/CPU
        devices=1
        if torch.cuda.is_available()
        else "auto",  # Use 1 GPU if available, otherwise default
