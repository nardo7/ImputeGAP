...
class MyData(PandasDataset):
    def __init__(self, data, mask_tr):
        df, dist, mask = self.load(data=data, mask_tr=mask_tr)
        self.dist = dist
    ...
    def load(self, data=None, mask_tr=None):
...
class MissingValuesMyData(MyData):
    def __init__(self, data, tr_mask, seed=42, p_fault=0.0015, p_noise=0.05):
        super(MissingValuesMyData, self).__init__(data, tr_mask)
