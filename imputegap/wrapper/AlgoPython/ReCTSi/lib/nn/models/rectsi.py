...
from imputegap.wrapper.AlgoPython.ReCTSi.lib.nn.models.Mytransformer import (
    SelfAttentionLayer,
)
...
class ReCTSINet(nn.Module):
    def __init__(self, d_in):
        ...
        self.node_emb = nn.Parameter(torch.empty(self.num_nodes, self.spatial_emb_dim))
        self.adp_emb = nn.Parameter(
            torch.empty(self.window_size, self.num_nodes, self.adp_emb_dim)
        )

        nn.init.xavier_uniform_(self.node_emb)
        nn.init.xavier_uniform_(self.adp_emb)
        self.output_proj = nn.Linear(
            self.window_size * self.model_dim2, self.window_size
        )
    ...
    def forward(self, x, mask, adj=None, pos=None, timestamp=None, adj_label=None):
        ...
        input_data_1 = masked_x.unsqueeze(-1)
        input_data_2 = init_x[..., third_of_last_dim : third_of_last_dim * 2].unsqueeze(
            -1
        )
        input_data_3 = init_x[..., third_of_last_dim * 2 :].unsqueeze(-1)
        ...
        spatial_emb = self.node_emb.expand(
            batch_size, self.window_size, *self.node_emb.shape
        )
        features.append(spatial_emb)
        adp_emb = self.adp_emb.expand(size=(batch_size, *self.adp_emb.shape))
        features.append(adp_emb)
