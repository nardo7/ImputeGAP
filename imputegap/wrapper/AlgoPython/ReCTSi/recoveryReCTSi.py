...
from imputegap.wrapper.AlgoPython.ReCTSi.lib.data.imputation_dataset import (
...
def get_model_classes(model_str):
    ...
    if model_str == "rectsi":
        model, filler = models.ReCTSINet, fillers.Filler
    else:
...
def recoveryReCTSi(
    ...
    logdir = os.path.join(config["logs"], "rectsi", exp_name)
    ...
    if len(torch_dataset) <= 1 or len(test_idxs) <= 1:
        print(
            f"\n(ERROR) Number of series to train to small for ReCTSi: {len(test_idxs)}\n\tPlease increase the number of series or change the dataset used.\n"
        )
        return input
    ...
    filler_kwargs = parser_utils.filter_args(
        args={**additional_filler_hparams},
